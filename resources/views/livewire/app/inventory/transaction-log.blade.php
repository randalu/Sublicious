<div>
    ...
    <div class="flex flex-wrap gap-3 mb-5">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input wire:model.live.debounce.300ms="search" type="search" placeholder="Search by item or notes…"
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($transactions as $tx)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{{ $tx->created_at->format('d M Y H:i') }}</td>
                            <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $tx->inventoryItem->name ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <span class="text-xs px-2 py-1 rounded-full {{ $typeColors[$tx->type] ?? 'bg-gray-100 text-gray-600' }}">{{ ucfirst($tx->type) }}</span>
                            </td>
                            <td class="px-4 py-3 text-right text-sm font-medium text-gray-900">
                                @if($tx->type === 'restock')
                                    <span class="text-green-600">+{{ number_format($tx->quantity, 2) }}</span>
                                @elseif(in_array($tx->type, ['deduction', 'waste']))
                                    <span class="text-red-600">-{{ number_format($tx->quantity, 2) }}</span>
                                @else
                                    {{ number_format($tx->quantity, 2) }}
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right text-sm text-gray-500">{{ number_format($tx->quantity_before, 2) }}</td>
                            <td class="px-4 py-3 text-right text-sm text-gray-500">{{ number_format($tx->quantity_after, 2) }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 max-w-[200px] truncate">{{ $tx->notes ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-500">{{ $tx->user?->name ?? '—' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
